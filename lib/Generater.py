...
def culculate_op(data: dict):
    ...
    with codecs.open(f'{cwd}/artifacter/Assets/duplicate.json', 'r', encoding='utf-8') as f:
        dup = json.load(f)
    with codecs.open(f'{cwd}/artifacter/Assets/subopM.json', 'r', encoding='utf-8') as f:
        mapping = json.load(f)
...
def generation(data):
    ...
    if CharacterName in ['蛍', '空']:
        CharacterImage = Image.open(
            f'{cwd}/artifacter/character/{CharacterName}({element})/avatar.png').convert("RGBA")
    else:
        if CharacterCostume:
            CharacterImage = Image.open(
                f'{cwd}/artifacter/character/{CharacterName}/{CharacterCostume}.png').convert("RGBA")
        else:
            CharacterImage = Image.open(
    ...
    if CharacterName == 'アルハイゼン':
        CharacterAvatarMask2 = Image.open(
            f'{cwd}/artifacter/Assets/Alhaitham.png').convert('L').resize(CharacterImage.size)
    else:
        CharacterAvatarMask2 = Image.open(
            f'{cwd}/artifacter/Assets/CharacterMask.png').convert('L').resize(CharacterImage.size)
    ...
    # 武器
    Weapon = Image.open(
        f'{cwd}/artifacter/weapon/{WeaponName}.png').convert("RGBA").resize((128, 128))
    ...
    WeaponRImage = Image.open(
        f'{cwd}/artifacter/Assets/Rarelity/{WeaponRarelity}.png').convert("RGBA")
    ...
    # 天賦
    TalentBase = Image.open(f'{cwd}/artifacter/Assets/TalentBack.png')
    ...
    for i, t in enumerate(['通常', 'スキル', "爆発"]):
        ...
        Talent = Image.open(
            f'{cwd}/artifacter/character/{CharacterName}/{t}.png').resize((50, 50)).convert('RGBA')
    ...
    # 凸
    CBase = Image.open(
        f'{cwd}/artifacter/命の星座/{element}.png').resize((90, 90)).convert('RGBA')
    Clock = Image.open(
        f'{cwd}/artifacter/命の星座/{element}LOCK.png').resize((90, 90)).convert('RGBA')
    ...
    for c in range(1, 7):
        if c > CharacterConstellations:
            ...
            CharaC = Image.open(
                f'{cwd}/artifacter/character/{CharacterName}/{c}.png').convert("RGBA").resize((45, 45))
    ...
    FriendShipIcon = Image.open(
        f'{cwd}/artifacter/Assets/Love.png').convert("RGBA")
    ...
    BaseAtk = Image.open(
        f'{cwd}/artifacter/emotes/基礎攻撃力.png').convert("RGBA").resize((23, 23))
    ...
    if WeaponSubOPKey != None:
        BaseAtk = Image.open(
            f'{cwd}/artifacter/emotes/{WeaponSubOPKey}.png').convert("RGBA").resize((23, 23))
        BaseAtkmask = BaseAtk.copy()
    ...
    if ScoreTotal >= 220:
        ScoreEv = Image.open(f'{cwd}/artifacter/artifactGrades/SS.png')
    elif ScoreTotal >= 200:
        ScoreEv = Image.open(f'{cwd}/artifacter/artifactGrades/S.png')
    elif ScoreTotal >= 180:
        ScoreEv = Image.open(f'{cwd}/artifacter/artifactGrades/A.png')
    else:
        ScoreEv = Image.open(f'{cwd}/artifacter/artifactGrades/B.png')
    ...
    for i, parts in enumerate(['flower', "wing", "clock", "cup", "crown"]):
        ...
        Preview = Image.open(
            f'{cwd}/artifacter/Artifact/{details["type"]}/{parts}.png').resize((256, 256))
        ...
        Pmask = Image.open(
            f'{cwd}/artifacter/Assets/ArtifactMask.png').convert('L').resize(Preview.size)
        ...
        MainIcon = Image.open(
            f'{cwd}/artifacter/emotes/{mainop}.png').convert("RGBA").resize((35, 35))
        ...
        for a, sub in enumerate(details['sub']):
            ...
            SubIcon = Image.open(
                f'{cwd}/artifacter/emotes/{SubOP}.png').convert("RGBA").resize((30, 30))
        ...
        if Score >= PointRefer[parts]['SS']:
            ScoreImage = Image.open(f'{cwd}/artifacter/artifactGrades/SS.png')
        elif Score >= PointRefer[parts]['S']:
            ScoreImage = Image.open(f'{cwd}/artifacter/artifactGrades/S.png')
        elif Score >= PointRefer[parts]['A']:
            ScoreImage = Image.open(f'{cwd}/artifacter/artifactGrades/A.png')
        else:
            ScoreImage = Image.open(f'{cwd}/artifacter/artifactGrades/B.png')

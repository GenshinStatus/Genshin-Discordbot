...
async def getCharacterImage(uid,id):
    ...
    for n in chara["equipList"]:
        ...
        if 'weapon' in n:
            #武器アイコン追加
            temp_4 = downloadPicture(f'https://enka.network/ui/{n["flat"]["icon"]}.png')
            ...
            #聖遺物UI追加
            temp_3 = downloadPicture(f'https://enka.network/ui/{n["flat"]["icon"]}.png')
            ...
            #メインステージ名　文字追加
            player_name = genshinTextHash[n["flat"]["reliquaryMainstat"]["mainPropId"]]
            font_size = 20
            font_color = (255, 255, 255)
            ...
            for b in n["flat"]["reliquarySubstats"]:
                ...
                width = 1650
                img = add_text_to_image(img, player_name, font_size, font_color, height, width)
                statscore.append({genshinTextHash[b["appendPropId"]]:b["statValue"]})

            getscore = {genshinTextHash[n["flat"]["reliquaryMainstat"]["mainPropId"]]:n["flat"]["reliquaryMainstat"]["statValue"]}
            #スコア値　文字追加
